...
MODULE_SCOPE int tclpkcs11_inittoken(ClientData cd, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[]) {
  struct tclpkcs11_interpdata *interpdata;
  struct tclpkcs11_handle *handle;
  Tcl_HashEntry *tcl_handle_entry;
  Tcl_Obj *tcl_handle, *tcl_slotid, *tcl_password, *tcl_tokenlab;
  long slotid_long;
  char *password;
  int password_len;
  ...
  if (objc != 5) {
    Tcl_SetObjResult(interp, Tcl_NewStringObj("wrong # args: should be \"pki::pkcs11::inittoken handle slot password label\"", -1));
...
int Tclpkcs11_Init(Tcl_Interp *interp) {
  ...
  tclCreatComm_ret = Tcl_CreateObjCommand(interp, "pki::pkcs11::inittoken", tclpkcs11_inittoken, interpdata, NULL);
  if (!tclCreatComm_ret) {
    return(TCL_ERROR);
  }
  ...
  tclCreatComm_ret = Tcl_CreateObjCommand(interp, "pki::pkcs11::inituserpin", tclpkcs11_inituserpin, interpdata, NULL);
  if (!tclCreatComm_ret) {
    return(TCL_ERROR);
  }
